...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sg="http://www.simpliccity.org/schema/styleguide">
	...
	<xsl:template match="sg:category" mode="toc">
		<ul>
			<li><a><xsl:attribute name="href">#<xsl:call-template name="formatCategoryAnchor" /></xsl:attribute><span><xsl:value-of select="@name" /></span></a></li>
			<xsl:if test="sg:topics">
				<ul><xsl:apply-templates mode="toc" /></ul>
			</xsl:if>
		</ul>
	</xsl:template>
	
	<xsl:template match="sg:topic" mode="toc">
			<li><a><xsl:attribute name="href">#<xsl:call-template name="formatTopicAnchor" /></xsl:attribute><span><xsl:value-of select="@name" /></span></a></li>
	</xsl:template>
